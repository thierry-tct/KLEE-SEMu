...
public:
  ...
  // original and mutant
  void ks_checkMaxSat (ConstraintSet const &mutPathCond,
  ...
  void ks_loadKQueryConstraints(std::vector<ConstraintSet> &outConstraintsList);

  void ks_CheckAndBreakInfinitLoop(ExecutionState &curState, ExecutionState *&prevState, time::Point &initTime);
  ...
  void ks_oldNewPrepareModule (llvm::Module *mod);
