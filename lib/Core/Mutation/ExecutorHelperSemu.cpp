...
void ExecutorHelperSemu::ks_mutationPointBranching(ExecutionState &state, 
  ...
  if (MaxForks!=~0u && stats::forks >= MaxForks) {
    ...
    for (std::vector<uint64_t>::iterator it = mut_IDs.begin(),
      ...
      std::pair<PTree::Node*,PTree::Node*> res = parentE->processTree->split(state.ptreeNode, ns, &state);
...
bool ExecutorHelperSemu::ks_checkfeasiblewithsolver(ExecutionState &state, ref<Expr> bool_expr) {
  bool result;
  bool success = parentE->solver->mayBeTrue(state, bool_expr, result);
...
#ifdef SEMU_RELMUT_PRED_ENABLED
inline bool ExecutorHelperSemu::ks_reachedAnOldNew(KInstruction *ki) {
  if (ki->inst->getOpcode() == Instruction::Call) {
    Function *f = dyn_cast<CallInst>(ki->inst)->getCalledFunction();
...
bool ExecutorHelperSemu::ks_diffTwoStates (ExecutionState *mState, ExecutionState *mSisState, 
  ...
  bool result;
  bool success = parentE->solver->mayBeTrue(*mState, origpathPrefix, result);
  assert(success && "KS: Unhandled solver failure");
  (void) success;
  if (result) {
...
bool ExecutorHelperSemu::ks_mutantPrePostDiff (ExecutionState *mState, bool outEnvOnly, 
  ...
  for (auto *s: parentE->states) {
    if (s->ks_mutantID == mState->semuESHelper.ks_mutantID && s->ks_old_new < 0) 
...
// original and mutant
void ExecutorHelperSemu::ks_checkMaxSat (ConstraintManager const &mutPathCond,
  ...
  if (! stateDiffExprs.empty()) {
    ...
    pmaxsat_solver.setSolverTimeout(parentE->coreSolverTimeout);
...
void ExecutorHelperSemu::ks_loadKQueryConstraints(std::vector<ConstraintManager> &outConstraintsList) {
...
void ExecutorHelperSemu::ks_CheckAndBreakInfinitLoop(ExecutionState &curState, ExecutionState *&prevState, double &initTime) {
...
void ExecutorHelperSemu::ks_odlNewPrepareModule (llvm::Module *mod) {
  // - Set isold global
...
void ExecutorHelperSemu::ks_oldNewBranching(ExecutionState &state) {
  ...
  if (MaxForks!=~0u && stats::forks >= MaxForks) {
    ...
    parentE->executeMemoryOperation (*ns, true, evalConstant(ks_isOldVersionGlobal), ConstantExpr::create(1, 1), 0);    // isOld is a boolean (1 bit int). 1 for True
    parentE->executeMemoryOperation (state, true, evalConstant(ks_isOldVersionGlobal), ConstantExpr::create(0, 1), 0);    // isOld is a boolean (1 bit int). 0 for False
