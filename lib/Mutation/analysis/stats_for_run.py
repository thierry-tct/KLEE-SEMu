...
def loadData(proj2dir, use_func=False, projcommonreldir=None, \
    ...
    merged_df['_precondLength'].replace(-1,'MD2MS'.lower(),inplace=True)
    merged_df['_precondLength'].replace(-2,'MD2MSC'.lower(),inplace=True)
    merged_df['_precondLength'].replace('-1','MD2MS'.lower(),inplace=True)
    merged_df['_precondLength'].replace('-2','MD2MSC'.lower(),inplace=True)
...
def get_table_muts_tests(outdir, killed_muts_obj, mintests_obj):
    ...
    data += '\n' + ' & ' + ' & '.join(['{\\bf \\#Gen.} & {\\bf \\#Killed}'] * 3) + '\\\\'
    data += '\n' + ' & ' + ' & '.join(['{\\bf Tests} & {\\bf Mutants}'] * 3) + '\\\\'
    data += '\n' + '\\hline \\hline'
